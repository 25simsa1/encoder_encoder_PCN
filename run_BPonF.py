"""BPonF -- OBJECTIVE CONTROL: backprop (Adam) trained END-TO-END on the IDENTICAL energy F, with NO
latent relaxation anywhere in training.

WHY. The paper's central contrast (PC on F vs E1 backprop CLIP) changes learning rule + objective +
optimizer simultaneously (red-team finding). run_E1_lars_infonce.py factors the OPTIMIZER axis; THIS
script factors the OBJECTIVE axis: keep backprop+Adam (the E1 learning machinery) but swap the loss from
InfoNCE to the PC energy F, byte-matched to run_coupling_scale.py:
    F = 0.5*mean_B( A_CROSS*sum_k[mse(S_k-it_k)+mse(S_k-tt_k)] + A_GEN*[mse(dec_img(S)-igt)+mse(dec_txt(S)-tgt)] )
with S PINNED to the average-init the PC relaxation starts from: S_k = (it_k+tt_k)/2. There is NO
relax_full / relaxation loop anywhere in training; gradients flow end-to-end through S into BOTH encoders
AND (unlike E1) into the decoders and projections -- F contains the reconstruction terms, so the decoders
MUST train. The 2x2 the controls complete:
    PC driver: F + relax-then-step + LARS   |  E1:  InfoNCE + backprop + Adam
    E1L:       InfoNCE + backprop + LARS    |  BPonF (this): F + backprop + Adam

DECISION RULE (pre-registered):
  - BPonF crosses held-out (latent bar) where PC at matched N/epochs does not -> F is learnable by
    end-to-end gradient descent; the PC failure lives in the relax-then-step dynamics, not in F.
  - BPonF ALSO at chance held-out (and/or never fits train) -> the objective F itself does not induce
    transferable alignment at this scale; the PC negative is not specific to the PC rule.
The TRAIN-FIT GATE (train lat_retr >= BPF_GATE_T=0.95) is a MEASURED OUTCOME, never a void: readouts
ALWAYS run and fit_gate_pass is recorded true/false. F was never designed as an InfoNCE substitute --
whether BP can fit train through F is itself part of the measurement. No early stop: the full epoch
budget runs so the rung mirrors the PC matched-epochs runs (E3b).
...
MEASURE (byte-matched to run_coupling_scale.py so numbers are comparable):
  ...
  i2t -- using the same GEN_INFER=25 relax_mono at READOUT TIME ONLY (readout-side relaxation is part of
  the shared readout definition for every arm, PC included; it is not training). Weight movement % is
  recorded exactly like the PC driver (reported, never a void condition here).

ENV: RUNS1_NTRAIN(2000) RUNS1_NEVAL(1000) RUNS1_PAIRS(auto) RUNS1_RES(64) RUNS1_CAPLEN(64) RUNS1_WMUL(1.5)
RUNS1_COCO(train2017) RUNS1_DATA RUNS1_READB(128) RUNS1_READTRAIN(1500) RUNS1_BATCHJ(128 = PC joint batch)
RUNS1_CKPT RUNS1_SMOKE | BPF_SEEDS("0,1,2") BPF_LR(3e-4 Adam, ramped) BPF_EPOCHS(150 = PC matched-epochs)
BPF_RAMP(300 = PC ramp) BPF_EVAL_EVERY(100 steps: train lat_retr tracking) BPF_GATE_T(0.95).
OUT: appends one record per (n_train, seed) to BPonF_results.json; weights to RUNS1_CKPT/bpf_seed{s}.npz
(same key layout as the PC cs_*.npz, so analysis_move_decomp.py applies).
"""
...
BATCHJ = int(os.environ.get("RUNS1_BATCHJ", 2 if SMOKE else 128))          # same knob as the PC joint phase
CKPT   = os.environ.get("RUNS1_CKPT", "/tmp/bpf_ckpt" if SMOKE else "/root")
SEEDS  = [int(s) for s in os.environ.get("BPF_SEEDS", "0" if SMOKE else "0,1,2").split(",")]
...
# recipe constants (identical to run_coupling_scale.py; N_INFER absent BY DESIGN -- no training relaxation)
HEADS, NBLK, NS = 4, 4, 4
CODE, DEC_SD = 16, 1e-3
A_CROSS, A_GEN = 1.0, 2.0
REL_C = 0.05
GEN_INFER = 3 if SMOKE else 25                                             # READOUT-ONLY relaxation depth
DIVERGE_W = 1e3
...
def make_ops(P,c,V):
    ...
    return dict(enc_img=enc_img,enc_txt=enc_txt,dec_img=dec_img,dec_txt=dec_txt,F_energy=F_energy,
                relax_mono=relax_mono,get_taps=get_taps,latents=latents)
...
def run_seed(seed):
    ...
    print(f"\n----- BPonF seed {seed}: train={NTR} eval={NEV} V={V} params={NP/1e6:.1f}M | Adam lr={LR} batchj={BATCHJ} "
          f"epochs={EPOCHS} ({total} steps, ramp {RAMP}) | NO training relaxation | chance eval={1/max(NEV,1):.5f}", flush=True)

    # ---- grad-coverage probe: under F, EVERY tensor (decoders included) must receive gradients ----
    ...
    with tf.GradientTape() as t0_:
        ...
        S0 = [0.5*(it0[k]+tt0[k]) for k in range(NS)]                 # the PC average-init, pinned (no relaxation)
        F0 = ops["F_energy"](S0, it0, tt0, igt0, tgt0, tf.reduce_mean)
    ...
    print(f"  grad coverage: {len(TV)}/{len(all_w)} tensors receive non-None grads | "
          f"decoder/proj tensors with grads: {len(dec_grads)}/{len(dec_names)} -> {sorted(dec_grads)}", flush=True)
    assert len(dec_grads) == len(dec_names), "BPonF requires the decoders/projections to receive gradients through F"
    ...
    def bpf_step(x, tk, igt, tgt, lr, t):
        with tf.GradientTape() as tp:
            ...
            S = [0.5*(it[k]+tt[k]) for k in range(NS)]                # forward-only S; identical to the PC relax START point
            F = ops["F_energy"](S, it, tt, igt, tgt, tf.reduce_mean)  # end-to-end: encoders + projections + decoders
        gr = tp.gradient(F, TV)
    ...
    try: np.savez(os.path.join(CKPT, f"bpf_seed{seed}.npz"), **{k: P[k].numpy() for k in P})
    except Exception: pass
    ...
    rec = dict(n_train=NTR, n_eval=NEV, seed=seed, coco=("smoke" if SMOKE else COCO), params=NP, V=V,
               ...
               optimizer="adam", objective="F_energy", relaxation_in_training=False)
...
# ============================ MAIN ============================
print(f"=== BPonF OBJECTIVE CONTROL (backprop Adam on the PC energy F, NO training relaxation) === "
      f"smoke={SMOKE} COCO={COCO} n_train={N_TRAIN} n_eval={N_EVAL} seeds={SEEDS} lr={LR} epochs={EPOCHS} "
      f"| bar: held-out lat_retr > 3/{N_EVAL} | fit gate = outcome, not void", flush=True)
records = [run_seed(s) for s in SEEDS]

# append-merge results (atomic write so a crash cannot corrupt earlier rungs)
out_path = os.path.join(HERE, "BPonF_results.json")
try:
...
print(f"\nsaved: BPonF_results.json (+{len(records)} records, total {len(existing['records'])})", flush=True)

# ============================ RUNG VERDICT ============================
ok      = [r for r in records if not r["diverged"]]
crossed = [r for r in ok if r["hits"] > 3]                                 # PRIMARY: latent bar, matched to E1/PC
gen_x   = [r for r in ok if r["gen_heldout"] and r["gen_heldout"]["hits"] > 3]
print(f"\n==================== BPonF RUNG VERDICT (n_train={N_TRAIN}, held-out only) ====================", flush=True)
for r in records:
    if r["diverged"]:
        ...
        print(f"  seed {r['seed']}: fit_gate={'PASS' if r['fit_gate_pass'] else 'FAIL'} (train {r['train_lat_retr']:.3f}, "
              ...
              f"({r['sigma']:+.1f} sigma) -> {'PASS' if r['hits']>3 else 'FAIL'} vs bar >3/{r['n_eval']} | "
              f"secondary gen retr {g['hits']}/{r['n_eval']} -> {'PASS' if g['hits']>3 else 'FAIL'}", flush=True)
def _hitlist(rs_): return ", ".join("seed %d: %d/%d" % (r["seed"], r["hits"], r["n_eval"]) for r in rs_)
if not ok:
    print("VERDICT: VOID -- all seeds diverged (numerics, not an objective-axis finding). Lower BPF_LR and rerun.", flush=True)
elif len(crossed) >= max(2, (len(ok)+1)//2):
    print(f"VERDICT: BP-ON-F CROSSES at n_train={N_TRAIN} ({_hitlist(crossed)}; gen bar: {len(gen_x)}/{len(ok)}). "
          f"The energy F IS learnable by end-to-end gradient descent -- if PC at matched N/epochs is at chance, "
          f"the PC failure lives in the relax-then-step dynamics, NOT the objective.", flush=True)
else:
    nofit = [r for r in ok if not r["fit_gate_pass"]]
    tail = (f" Fit gate also failed on {len(nofit)}/{len(ok)} seed(s) (train lat_retr: "
            f"{', '.join('%.3f' % r['train_lat_retr'] for r in nofit)}) -- measured outcome, recorded, not voided."
            if nofit else " All seeds fit train, so the objective transfers no better under BP than under PC.")
    print(f"VERDICT: BP-ON-F AT CHANCE at n_train={N_TRAIN} -- {_hitlist(ok)} (bar >3/{N_EVAL}). "
          f"The objective F does not induce transferable alignment even under backprop+Adam; the PC negative is "
          f"not specific to the PC rule.{tail}", flush=True)
if SMOKE: print("\n[SMOKE] mechanics-only; numbers meaningless. Confirms no-relax training path, decoder grads, "
                "Adam-on-F loop, PC-matched readouts (latent + generation), gate-as-outcome, json, verdict.", flush=True)
