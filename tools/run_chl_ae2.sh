...
# latent-AE CHL with an EXPRESSING free phase: image-set latents (well-posed target) and the
# free/anti-learn sample run top-down-dominant at an adequate rate, so the contrast calibrates
# the composed top-down inverse instead of seeing a rate-starved zero sample
python3 train_coco64.py --config coco64_gen --train-mode chl --gen-latents image \
  --free-pi-bu 0.0 --free-state-lr 0.25 --gen-relax-k1 45 \
  --gen-lr 3e-4 --gen-every 4 --weight-norm --pairs "${PAIRS:-2000}" --epochs "${EPOCHS:-12}" \
